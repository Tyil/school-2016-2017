...
effect, Funtoo, a hard task to complete. Some people have also shown interest
in my particular setup.

As such, I have written this tutorial to show people my installation steps. If
you have any suggestions or criticism, please find me on IRC. The networks I
frequent and the nickname I use can be found on [my homepage][tyil].
...
it, however, there might be much jargon you do not understand, making the
...
typing `o` and pressing enter. When asked whether you are sure, type `y` and
enter again.
...
definitely important. We are going to encrypt the entire `lvm` partition using
[`luks`][wikipedia-luks]. The front end tool to be used for this is
...
`cryptsetup` will ask you for a pass phrase. Make sure to use a good one,
...
sub shell is called to figure out the size of the swap partition.
...
Next up is mounting all file systems so you can install files to them. First, you
...
Now you can add some directories for the other mount points. This can be done in
...
Let's also enable swap and ram disks for the temporary storage directories:
...
### Initial setup
Now that all mount points have been set up, installation of the actual OS can
...
is easiest to download and extract the tarball in the root file system, so let's
do that:
...
Once these mount points are set, you will need to copy over `resolv.conf` so the
...
favorite editor. Funtoo comes with [`vi`][wikipedia-vi],
...
Every time you want to update your system, you will have to do an `emerge --sync`
...
information on your mount points. Some of the mount points are best configured
...
a directory. This way, you can split off your configurations into multiple files for
easier maintenance. The files will be loaded alphabetically. The way you set it
...
around some man pages to find out what you exactly want yourself.
...
**incredibly** important to not use newlines in these two, as [they will break
...
latest official kernel without the gentoo patch set, pick
...
[`grub`][wikipedia-grub], the recommended boot loader for Funtoo. It doesn't
require much configuration thanks to the `boot-update` script, which will
configure `grub` for you.

Before running the script, there's one place to update as this setup uses `luks`
and `lvm`.

Open up `/etc/boot.conf` in your favorite editor and let the file display
something like this:
...
Now that `boot-update` is configured, install `grub` as an UEFI boot loader and
generate the configurations for it using `boot-update`.
...
#### Set passwords
We probably want to be able to log in to the system as well. By default, users
...
If you used a different user name than `tyil`, be sure to change it here as well.
...
If you set your UEFI to favor the USB system over the standard drive in the
...
If you need assistance on maintenance, you can always drop by in `#sqt` on
[Gratisnode][freenode].
